...
{
    /// <summary>
    /// an abstract class which implements ILogger interface with few abstract methods implemented in platform specific projects
    /// </summary>
    public abstract class LoggerBase : ILogger
    {
        ...
        public abstract bool AppendToFile(string filename, string message);

        public abstract bool SaveToFile(string filename, string message);

        public abstract string LoadFromFile(string filename);

        public abstract bool DeleteFile(string filename);

        public abstract string GetNextAvailableFileName(string logFileNameBase);

        public abstract long GetFileSizeKb(string fileName);
        ...
        private void LogRecord(string message, Exception exc = null)
        {
            string rec = "";
            if (exc == null)
                rec = string.Format("{0} - {1}", DateTime.Now + "( " + TimeZoneInfo.Local.StandardName + " )", message);
            else
                rec = string.Format("{0} - {1}. EXCEPTION: {2}. STACK TRACE: {3}.", DateTime.Now + "( " + TimeZoneInfo.Local.StandardName + " )", message, exc.Message, exc.StackTrace != null ? exc.StackTrace.ToString() : "");
        ...
        /// </summary>        
        /// <param name="logFileNameBase">The log file name without extension. Logger will add a suffix to ensure uniqueness of the filename.</param>
